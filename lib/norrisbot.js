...
NorrisBot.prototype._replyWithRandomJoke = function (originalMessage) {
	...
	if (originalMessage.subtype && originalMessage.subtype === 'channel_join') {
		var newUser = self.users.filter(function (user) {
        	return user.id === originalMessage.user;
    	})[0];

    	var welcome = "Thanks for joining Angular Buddies. Check out our FAQs and Code of Conduct at <http://www.github.com|GitHub>.  " +
		"If you're brand new to Angular, post questions to <#" + originalMessage.channel + "|Random>, otherwise, post to <#random>."
		
		self.postMessageToUser(newUser.name, welcome, {as_user:true}, function(){
